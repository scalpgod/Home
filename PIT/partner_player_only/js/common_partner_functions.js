var site_url = "https//www.livesquawk.com/";
...
function set_static_session_info() {
    ...
    session_info["session_email"] = $("#username_value").text();
}

function partner_start_without_click() {
...
function comfun_play_popup_flash() {
    $(document).on("click", ".header__player__strip__button--flash", function(event) {
        ...
        check_boot();
        load_script_with_callback(site_url + "partner_player_only/js/player_flash_tradingfloor.js?v=7", comfun_flash_js_loaded);
    });
}
...
function comfun_play_pop_webrtc() {
    $(document).on("click", ".header__player__strip__button--webrtc", function(event) {
        ...
        check_boot();
        load_script_with_callback(site_url + "partner_player_only/js/player_webrtc_tradingfloor.js?v=8", comfun_webrtc_js_loaded);
    });
}
...
function comfun_new_webrtc_js_loaded() {
    socket_connection_start(distributor_server);
    $(".webrtc__player__outer").show();
    $(".header__player__strip--choice").hide();
    $("#user_agent").html(capitalize(adapter.browserDetails.browser));
